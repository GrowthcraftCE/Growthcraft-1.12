...
public class BeesRegistry
{
	...
	{
		GrowthcraftBees.logger.debug("Adding Bee {%s}:%d", bee, meta);
	...
	public void addBee(@Nonnull ItemStack stack)
	{
		...
		GrowthcraftBees.logger.debug("Adding Bee {%s}", key);
		beesList.add(key);
	}
	...
	{
		GrowthcraftBees.logger.debug("Adding Honey Comb mapping {%s} - {%s}", empty, full);
	...
	{
		GrowthcraftBees.logger.debug("Adding Flower {%s}:{%s}", key, entry);
